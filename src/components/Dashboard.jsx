...
//         `https://us1.locationiq.com/v1/search.php?key=YOUR_NEW_API_KEY&q=${encodeURIComponent(address)}&format=json` // Replace with valid key
//       );
//       if (!response.ok) {
...
//       console.log(`Coordinates response for ${locationType}:`, data);
...
//     try {
//       const response = await fetch(
//         `https://us1.locationiq.com/v1/autocomplete.php?key=pk.4ebcc4fec220d7040d681509bbb07232&q=${encodeURIComponent(query)}&format=json&limit=5` // Replace with valid key
//       );
//       if (!response.ok) {
//         if (response.status === 401) {
//           throw new Error('enter location');
...
//       }
//       const data = await response.json();
//       console.log(`Suggestions response for ${locationType}:`, data);
//       setSuggestions(prev => ({
//         ...prev,
//         [key]: data
//       }));
...
//         console.log('New Booking ID:', newBooking.id || newBooking._id); // Log the booking ID
...
const Dashboard = () => {
  ...
  const calculateQuote = async () => {
    try {
      const response = await fetch(
        `http://localhost:5000/api/calculate-quote?serviceType=${quoteFormData.serviceType}&vehicleType=${quoteFormData.vehicleType}&cargoQuantity=${parseInt(quoteFormData.cargoDetails.match(/(\d+)/)[1])}&pickupLocation=${quoteFormData.pickupLocation}&deliveryLocation=${quoteFormData.deliveryLocation}`,
        {
          headers: { 'Authorization': `Bearer ${token}` }
        }
      );
